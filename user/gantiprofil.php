...
$name_err = $tlp_err = $name = $tlp = $alamat = $alamat_err = $fotoprofil[] = $fotoprofil_err = "";
...
if(empty($name_err) && empty($tlp_err) && empty($alamat_err)){
        
        // Prepare an insert statement
        $sql = "UPDATE users SET name=?, tlp=?, alamat=?, fotoprofil=? WHERE username = '".$usernama."'";
       
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss",  $param_name, $param_tlp, $param_alamat, $new_filename[0]);
...
$sql = "SELECT * FROM users WHERE username = '".$usernama."'";
                        ...
                        <div class="login-content">
                            ...
                            <div class="login-form">
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data" method="post">
									<img id="output1" alt="Foto Profil" height="100px" width="100px" src="<?php echo "../uploadfotoprofil/"; if(empty($row['fotoprofil'])) { echo "default.jpg"; } else { echo $row['fotoprofil']; }?>">
                                            <input placeholder="Foto Profil" onchange="loadFile1(event)" type="file" name="fotoprofil[]" multiple="multiple">
											<script>
									...
									<input name="name" placeholder="Nama Lengkap" type="text" class="form-control" value="<?php echo $name; ?>" ><span class="help-block"><?php echo $name_err; ?></span>
									<span class="help-block">Alamat :</span>
									<input name="alamat" placeholder="Nama Lengkap" type="text" class="form-control" value="<?php echo $alamat; ?>" ><span class="help-block"><?php echo $alamat_err; ?></span>
									
									<span class="help-block">Telepon :</span>
									<input name="tlp" placeholder="Nomor Telepon" type="text" class="form-control" value="<?php echo $tlp; ?>" ><span class="help-block"><?php echo $tlp_err; ?></span>
